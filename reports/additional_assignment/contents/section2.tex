...
\begin{itemize}
    \item \textbf{Tập trạng thái ẩn $S$}: $S = \{s_1, s_2, \ldots, s_N\}$ với $N$ là số trạng thái. Trong bài toán của chúng ta, $N = 26$ trạng thái (24 hợp âm + N + Complex).
    
    \item \textbf{Tập quan sát $O$}: Không gian các giá trị quan sát có thể. Trong trường hợp này, mỗi quan sát là một vector chroma 12 chiều: $o_t \in \mathbb{R}^{12}$.
...
GMM cho phép mô hình hoá phân phối multimodal (nhiều đỉnh) bằng cách kết hợp nhiều Gaussian thành phần.
...
\begin{itemize}
    ...
    \item $\mu_{ik} \in \mathbb{R}^{12}$: vector trung bình của thành phần $k$
    \item $\Sigma_{ik} \in \mathbb{R}^{12 \times 12}$: ma trận hiệp phương sai (covariance matrix) của thành phần $k$
...
\begin{itemize}
    \item \texttt{n\_components=40}: số thành phần Gaussian (có thể tuning)
    \item \texttt{covariance\_type='full'}: ma trận hiệp phương sai đầy đủ (không giả định độc lập giữa các chiều)
